...
import { Mail, MessageSquare, Phone, Volume2, Send, Users, CreditCard, ShoppingCart, Trash2 } from "lucide-react";
...
const InvitationManagementTab = ({ event, eventId }) => {
  ...
  const { sendNotification, isLoading: sendingInvitations } = SendNotificationManager({ eventId });
  ...
  const guestGroups = eventsData?.data?.map(eventItem => ({
    ...
    <div className="space-y-6">
      ...
      {activeTab === "send" && (
        <div className="space-y-6">
          ...
          <div>
            <h3 className="text-lg font-medium text-gray-900 mb-4">Select Events to Send Invitations For</h3>
            {loadingEvents ? (
                ...
                <div key={group.id} className="border border-gray-200 rounded-lg">
                  <div className="flex items-center space-x-3 p-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-600">{group.count} guests • Date: {group.eventDate ? new Date(group.eventDate).toLocaleDateString() : 'Not set'}</p>
                    </div>
                  </div>
                </div>
                ))}
            ...
            <div className="bg-purple-50 p-4 rounded-lg">
              <div className="flex justify-between items-center">
                ...
                <Button 
                  ...
                  isLoading={sendingInvitations || updatingEvent || uploadingFile}
                  onClick={async () => {
                    ...
                    // Send invitations to selected event groups
                    // When sending from invitation management, we pass eventIds instead of inviteeIds
                    const notificationPayload = {
                      inviteeIds: [], // Empty for invitation management
                      eventIds: selectedGroups, // The selected event group IDs
