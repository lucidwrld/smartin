...
const SubscriptionsPage = () => {
    ...
    const { data: plansResponse, isLoading: plansLoading } = useGetPlansManager(
    ...
    } = useGetUserSubscriptionsManager({});

    const {
        purchaseSubscription,
        isLoading: purchasing,
        data: purchaseData,
    } = PurchaseSubscriptionManager();

    const { cancelSubscription, isLoading: canceling } =
        CancelSubscriptionManager();
    ...
    const { renewSubscription, isLoading: renewing } = RenewSubscriptionManager(
        { subscriptionId: currentSubscription?.id }
    );

    useEffect(() => {
        if (purchaseData) {
            window.location.href = purchaseData?.data?.checkoutUrl;
        }
    }, [purchaseData]);
    ...
    const handlePurchase = async (plan) => {
        try {
            ...
            await purchaseSubscription(purchasePayload);
        } catch (error) {
                    ...
                    <>
                        ...
                        <div className="mb-8 text-center">
                            ...
                            <p className="text-gray-600 mb-2">
                                Select the perfect plan for your event
                                management needs
                            </p>
                            <p className="text-sm text-gray-500">
                                Showing plans in {userCurrency} based on your
                                profile settings
                            </p>
                        </div>
                        ...
                        {plansLoading ? (
                            ...
                            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                                {plans.map((plan) => {
                                    ...
                                    const isBusinessPlan = plan.name
                                        ?.toLowerCase()
                                        .includes("business");
                        ...
                        <div className="mt-16 text-center">
                            ...
                            <div className="grid md:grid-cols-3 gap-8 mt-8">
                                ...
                                <div className="text-center">
                                    ...
                                    <p className="text-gray-600 text-sm">
                                        Access to all invitation channels and
                                        analytics
                                    </p>
                                </div>
                                <div className="text-center">
                                    ...
                                    <p className="text-gray-600 text-sm">
                                        Get help when you need it with priority
                                        support
                                    </p>
                                </div>
                            </div>
...
const SubscriptionManagement = ({
    ...
    const isExpiringSoon =
        expiry.getTime() - new Date().getTime() < 7 * 24 * 60 * 60 * 1000; // 7 days
